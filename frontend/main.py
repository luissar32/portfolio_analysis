...
def optimize_portfolio(returns, risk_free_rate):
    ...
    weights = np.ones(num_assets) / num_assets  # Pesos iguales
    # Generar frontera eficiente simplificada
    mean_returns = returns.mean() * 252
    std_returns = returns.std() * np.sqrt(252)
    # Evitar secuencias vacías o inválidas
...
st.markdown("Analiza y optimiza portafolios de inversión con datos financieros en tiempo real. Calcula métricas clave y visualiza la frontera eficiente.")
...
if start_date > end_date:
    ...
    if st.button("Analizar Portafolio"):
        ...
        st.info("Ingresa tickers o sube un CSV y haz clic en 'Analizar Portafolio' para comenzar.")
